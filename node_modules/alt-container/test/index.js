...
export default {
    AltContainer: {
        ...
        "many children get state via props"() {
            ...
            const strong = TestUtils.findRenderedDOMComponentWithTag(
                node,
                "strong"
            );
        ...
        "does not wrap if it does not have to"() {
            ...
            assert(
                node.props.children.type === "span",
                "single node does not wrap"
            );
            ...
            assert.ok(
                Array.isArray(many.props.children),
                "multiple nodes are wrapped"
            );
        },
        ...
        "function is called with props"() {
            ...
            assert.ok(
                storeFunction.calledTwice,
                "called twice, once for store listening and another for props"
            );
            assert(
                storeFunction.args[0].length === 1,
                "called with one parameter"
            );
            assert(
                storeFunction.args[1].length === 1,
                "called with one parameter"
            );
        ...
        "custom rendering"() {
            ...
            TestUtils.renderIntoDocument(<AltContainer render={render} />);
        ...
        "changing an already mounted components props"() {
            ...
            assert(
                node.refs.test.props.store === TestStore,
                "node gets first state"
            );

            cb({store: Store2});

            assert(
                node.refs.test.props.store === Store2,
                "node changes props properly"
            );
        },
        ...
        "inject actions"() {
            ...
            assert(
                span.props.MyActions === action,
                "MyActions is injected actions"
            );
            assert.isFunction(
                span.props.MyActions.sup,
                "sup action is available"
            );
        },
        ...
        "inject all actions directly shorthand"() {
            ...
            assert.isFunction(
                span.props.sup,
                "sup is available directly on the props"
            );
        },
        ...
        scu() {
            ...
            assert.ok(
                scu.calledOnce,
                "custom shouldComponentUpdate was called"
            );
            assert(
                scu.args[0].length === 3,
                "3 args are passed, the nextProps, nextState, and this.getProps()"
            );
        ...
        injectables() {
            ...
            assert(
                span.props.className === "foo",
                "you can inject custom things"
            );
            ...
            assert(
                span.props.foo.x === 888,
                "when passing stores as Array they are just listened on"
            );
        },
