...
(function(is) {
    ...
    const isFunctionOfType = type => value =>
        is.function_(value) &&
        is.function_(value.constructor) &&
    ...
    is.inRange = (value, range) => {
        ...
        if (is.array(range) && range.length === 2) {
            return (
                value >= Math.min.apply(null, range) &&
    ...
    const isAbsoluteMod2 = value => rem =>
        is.integer(rem) && Math.abs(rem % 2) === value;
    is.even = isAbsoluteMod2(0);
    is.odd = isAbsoluteMod2(1);
    const isWhiteSpaceString = value =>
        is.string(value) && /\S/.test(value) === false;
    const isEmptyStringOrArray = value =>
        (is.string(value) || is.array(value)) && value.length === 0;
    const isEmptyObject = value =>
        !is.map(value) &&
        !is.set(value) &&
        is.object(value) &&
        Object.keys(value).length === 0;
    const isEmptyMapOrSet = value =>
        (is.map(value) || is.set(value)) && value.size === 0;
    is.empty = value =>
        is.falsy(value) ||
        isEmptyStringOrArray(value) ||
        isEmptyObject(value) ||
        isEmptyMapOrSet(value);
    is.emptyOrWhitespace = value =>
        is.empty(value) || isWhiteSpaceString(value);
    const predicateOnArray = (method, predicate, args) => {
        const values = Array.prototype.slice.call(args, 1);
        if (is.function_(predicate) === false) {
            throw new TypeError(
                `Invalid predicate: ${util.inspect(predicate)}`
            );
        }
        if (values.length === 0) {
