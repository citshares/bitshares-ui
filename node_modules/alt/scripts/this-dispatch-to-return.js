...
const updateDispatchToCall = j => p => {
    j(p).replaceWith(
        j.callExpression(j.identifier("dispatch"), p.value.arguments)
    );
};

const updateToJustThis = j => p => {
    j(p).replaceWith(
        j.memberExpression(p.value.object.object, p.value.property)
    );
};
...
module.exports = (file, api) => {
    ...
    root.find(j.FunctionExpression).forEach(p => {
        // ignore constructors
        if (
            p.parent.value.type === "MethodDefinition" &&
        ...
        const withinParent = findDispatches(j, p)
            .filter(x => x.parent.parent.parent.value === p.value)
            .size();
        ...
        if (withinParent === 0 && dispatches > 0) {
            ...
            if (hasReturn) {
                console.warn(
                    "Could not transform function because it returned",
                    "at line",
                    p.parent.value.loc.start.line
                );
            } else {
                console.warn(
