...
var lzma, fs, p, params, mode, suffix;
...
function compress_files(files) {
    (function loop(i) {
        ...
        if (
            ...
            !params.f &&
            !params.force &&
            !(params.r || params.testComp) &&
            fs.existsSync(files[i] + suffix)
        ) {
                ...
                if (!params.q && !params.quiet) {
                    console.error(
                        p.basename(files[i]) +
                            " already has " +
                            suffix +
                            " suffix -- unchanged. Use -S to change suffix."
                    );
                }
                return loop(i + 1);
            }
        ...
        lzma.compress(
            is_file ? fs.readFileSync(files[i]) : files[i].toString(),
            mode,
...
function decompress_files(files) {
    (function loop(i) {
        ...
        if (is_file) {
            ...
            if (ext !== suffix) {
                if (!params.q && !params.quiet) {
                    console.error(
                        p.basename(files[i]) +
                            " unknown suffix -- unchanged. Use -S to change suffix."
                    );
                }
                return loop(i + 1);
            }
        ...
        if (
            ...
            !params.force &&
            !params.t &&
            fs.existsSync(p.basename(files[i], ext))
        ) {
            if (!params.q && !params.quiet) {
                console.error(
                    "File already exists. Use -f to force overwrite."
        ...
        lzma.decompress(
            typeof files[i] === "string" ? fs.readFileSync(files[i]) : files[i],
            function ondone(data) {
                var j, len, buf;
                ...
                if (params.c || params.stdout) {
                    ...
                    write_file(
                        p.basename(files[i], ext),
                        typeof data === "string" ? data : array2buffer(data),
                        files[i]
                    );
                    if (params.v || params.verbose) {
...
function is_compress() {
    return (
        ...
        params.r ||
        params.testComp ||
        !(params.d || params.decompress || params.t || params.test)
    );
}
