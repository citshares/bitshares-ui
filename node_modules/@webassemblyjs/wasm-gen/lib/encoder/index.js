...
var _helperWasmBytecode = _interopRequireDefault(
...
function encodeModuleImport(n) {
    ...
    switch (n.descr.type) {
        ...
        case "Memory": {
            ...
            out.push.apply(
                out,
                _toConsumableArray(encodeLimits(n.descr.limits))
            );
            break;
        }
        ...
        case "FuncImportDescr": {
            ...
            out.push.apply(
                out,
                _toConsumableArray(encodeU32(n.descr.id.value))
            );
            break;
        }
        ...
        default:
            throw new Error(
                "Unsupport operation: encode module import of type: " +
                    n.descr.type
            );
    }
...
function encodeModuleExport(n) {
    ...
    var exportTypeByteString =
        _helperWasmBytecode.default.exportTypesByName[n.descr.exportType];
...
function encodeInstr(n) {
    ...
    if (typeof byteString === "undefined") {
        throw new Error(
            "encodeInstr: unknown instruction " +
                JSON.stringify(instructionName)
        );
    }
...
function encodeElem(n) {
    ...
    var funcs = n.funcs.reduce(function(acc, x) {
        return _toConsumableArray(acc).concat(
            _toConsumableArray(encodeU32(x.value))
        );
    }, []);
