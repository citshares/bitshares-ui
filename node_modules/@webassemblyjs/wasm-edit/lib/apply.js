        ...
        (0, _ast.traverse)(ast, {
            Func: function Func(_ref3) {
                ...
                if (funcHasThisIntr === true) {
                    ...
                    var bodySizeDeltaBytes =
                        replacementByteArray.length - oldNodeSize;

                    if (bodySizeDeltaBytes !== 0) {
                        var newValue =
                            node.metadata.bodySize + bodySizeDeltaBytes;
...
function applyDelete(ast, uint8Buffer, node) {
    ...
    if (sectionName === "start") {
        ...
        uint8Buffer = (0, _helperWasmSection.removeSection)(
            ast,
            uint8Buffer,
            "start"
        );
    ...
    ops.forEach(function(op) {
        ...
        switch (op.kind) {
            case "update":
                ...
                sectionName = (0, _helperWasmBytecode.getSectionForNode)(
                    op.node
                );
                break;
        ...
        if (state.deltaBytes !== 0) {
            ...
            if (sectionName !== "start") {
                state.uint8Buffer = (0,
                _helperWasmSection.resizeSectionByteSize)(
                    ast,
                    state.uint8Buffer,
                    sectionName,
                    state.deltaBytes
