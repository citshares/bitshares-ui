    ...
    function walk() {
        ...
        function eatTokenOfType(type) {
            if (token.type !== type) {
                throw new Error(
                    "\n" +
                        (0, _helperCodeFrame.codeFrameFromSource)(
                            source,
                            token.loc
                        ) +
        ...
        function parseTable() {
            ...
            while (token.type !== tokens.closeParen) {
                ...
                if (lookaheadAndCheck(tokens.openParen, keywords.elem)) {
                    ...
                    lookaheadAndCheck(tokens.openParen, keywords.export)
                ) {
                    eatToken(); // (
            ...
            if (isKeyword(token, keywords.func)) {
                ...
                while (token.type === tokens.openParen) {
                    ...
                    if (lookaheadAndCheck(keywords.param) === true) {
                        ...
                        fnParams.push.apply(
                            fnParams,
                            _toConsumableArray(parseFuncParam())
                        );
                    } else if (lookaheadAndCheck(keywords.result) === true) {
                        eatToken();
                        fnResult.push.apply(
                            fnResult,
                            _toConsumableArray(parseFuncResult())
                        );
                    } else {
                        throw (function() {
                            return new Error(
                ...
                throw new Error(
                    "Unsupported import type: " + tokenToString(token)
                );
            }
                        ...
                        if (
                            ...
                            throw (function() {
                                return new Error(
                                    "\n" +
                                        (0,
                                        _helperCodeFrame.codeFrameFromSource)(
                                            source,
                                            token.loc
                                        ) +
                                        "\n" +
        ...
        function parseCallIndirect() {
            ...
            while (token.type !== tokens.closeParen) {
                if (lookaheadAndCheck(tokens.openParen, keywords.type)) {
                    ...
                    lookaheadAndCheck(tokens.openParen, keywords.param)
                ) {
                    eatToken(); // (
                    ...
                    lookaheadAndCheck(tokens.openParen, keywords.result)
                ) {
                    eatToken(); // (
        ...
        function parseExport() {
            if (token.type !== tokens.string) {
                throw new Error(
                    "Expected string after export, got: " + token.type
                );
            }
        ...
        function parseFuncInstrArguments(signature) {
            ...
            while (
                token.type === tokens.name ||
                isKeyword(token, keywords.offset)
            ) {
                var key = token.value;
                eatToken();
                ...
                if (token.type === tokens.number) {
                    ...
                    throw new Error(
                        "Unexpected type for argument: " + token.type
                    );
                }
            ...
            var signatureLength = signature.vector
                ? Infinity
                : signature.length;
            ...
            if (token.type === tokens.name || token.type === tokens.valtype) {
                ...
                if (token.type === tokens.closeParen) {
                    ...
                    if (typeof object === "undefined") {
                        return t.withLoc(
                            t.instruction(_name2),
                            _endLoc,
                            startLoc
                        );
                    } else {
                        return t.withLoc(
                            t.objectInstruction(_name2, object, []),
                            _endLoc,
                            startLoc
                        );
                    }
                ...
                if (typeof object === "undefined") {
                    return t.withLoc(
                        t.instruction(_name2, _args, _namedArgs),
                        endLoc,
                        startLoc
                    );
                } else {
