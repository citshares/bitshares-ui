...
In order to increase your exposure to CTS and offer liquidity to BitAssets, such
...
The CitShares network is capable of issuing any amount of any BitAsset and lend
it out to participants given enough collateral.

 * *settlement price*: The price for 1 CTS as it is traded on external exchanges.
...
The CitShares network is capable of margin calling those positions that do not
...
offer to buy the collateral is less than the call price (x/CTS).
...
market rates up to the SQP and close the position. The remaining CTS of the
...
increase, an additional amount of CTS is locked as collateral, while reducing
...
particular bitAsset to hand it over to the CitShares network. After that, the
